...
4. Use the deterministic calculator for all numeric tables.
   ...
   - For very small issues, apply a cooling check before raising thresholds: if both subscription cap cash and online issue shares are far below recent samples, pull the base scenario toward small-size comparables instead of blindly using recent high-water marks.
